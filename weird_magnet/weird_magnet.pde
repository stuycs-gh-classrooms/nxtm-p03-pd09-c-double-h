  ...
  if (toggles[MOVING]) {
    ...
    for (int o=0; o < orbCount - 1; o++) {
      ...
      if (toggles[MAGNET]){
        ...
        orbs[o].applyForce(orbs[o].getMagnet(orbs[o + 1], orbs[o].charge, 0.5));
      for (int test = orbCount -1 ; test > 0; test--){
        orbs[o].applyForce(orbs[o].getMagnet(orbs[test], orbs[o].charge, 0.5));
      }
